...
Patterns binary_suggest3(int ep, auto &patterns, std::vector<EncodedPat> &basic_pat, EncodedPat &opt_pat, auto &scores, std::pair<int, char> &pre_target, int &pre_val, int &nx_step)
{
    ...
    else
    {
            ...
            else
            {
                rep(i, M)
                {
                    if (i == pre_target.first)
                    {
                        ...
                        rep(x, X)
                        {
                            if (pre_target.second == 'A')
                                ret_pat[i][2 * x + 1] = opt_pat[i].second;
                            else if (pre_target.second == 'B')
                                ret_pat[i][2 * x] = opt_pat[i].first;
                        }
                        if (pre_target.second == 'A')
                            ret_enc_pat[i] = {nx, opt_pat[i].second};
                        else
                            ret_enc_pat[i] = {opt_pat[i].first, nx};
                    }
                    else
                    {
                        ret_pat[i] = std::string(2 * X, opt_pat[i].first);
        ...
        else
        {
            ...
            if (pre_target.second == 'A')
                opt_pat[pre_target.first].first = opt_val;
            else if (pre_target.second == 'B')
                opt_pat[pre_target.first].second = opt_val;
            ...
            else
            {
                rep(i, M)
                {
                    if (i == pre_target.first)
                    {
                        ...
                        {
                            if (pre_target.second == 'A')
                                ret_pat[i][2 * x + 1] = opt_pat[m_id].second;
                            else
                                ret_pat[i][2 * x] = opt_pat[m_id].first;
                        }
                        //ターゲット以外のもの
                        ret_enc_pat[i] = opt_pat[i];
                    }
            ...
            else
            {
                ...
                rep(i, M)
                {
                    if (i == nx_m_id)
                    {
                        ...
                        {
                            if (nx_ab != 'A')
                                ret_pat[i][x * 2] = opt_pat[i].first;
                            else
                                ret_pat[i][x * 2 + 1] = opt_pat[i].second;
                        }
                        ret_enc_pat[i] = {'5', '5'};
                        if (nx_ab != 'A')
                            ret_enc_pat[i].first = opt_pat[i].first;
                        else
                            ret_enc_pat[i].second = opt_pat[i].second;
                    }
                    else
                    {
                        ret_pat[i] = std::string(2 * X, opt_pat[i].first);
...
Pattern change_opt_dfs(int ep, std::vector<ll> &scores, std::vector<std::string> &base_pat, EncodedChange &pre_change_pat) {
