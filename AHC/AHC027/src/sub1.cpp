...
struct Input
{
  ...
  std::array<std::array<int, MAX_N>, MAX_N> dirty;
...
{
  int N;
  std::array<std::string, MAX_N> horizontals;
  std::array<std::string, MAX_N> verticals;
...
inline void Field::setInput(const Input &input)
{
  this->N = input.N;
  for (int i = 0; i < this->N - 1; i++)
  {
    this->horizontals[i] = input.horizontal[i];
  }
  for (int i = 0; i < this->N; i++)
  {
    this->verticals[i] = input.vertical[i];
  }
  for (int i = 0; i < this->N; i++)
  {
    ...
    {
      int idx = i * N + j;
      this->dirty[idx] = input.dirty[i][j];
    }
  }
} // setInput(const Input& input)
...
public:
  ...
  Commands findPathExcludeVisited() const;
...
Commands TakahashikunCleanerNo2::findPath(bool excludeVisited=false) const
{
  ...
  for(int pos = 0; pos < NUM_CELL; pos++)
  {
    ...
    double avgValue = (double)(-dp[pos]) / field->dist[from][pos];
    if (avgValue > highest)
    {
      highest = avgValue;
      to = pos;
  ...
  LOG_INFO("To: %4d, Highest: %8.2f", to, highest);
